...
class ProviderController extends Controller
{
    ...
    public function importDataProvider(Request $request)
    {
        ...
        if (!isset($lines[0])) {
            ...
            if ($success > 0) {
                return back()
                    ->with('error', $errors . " datos no se importaron correctamente. Asegúrate que los datos del reporte o tabla, no estén registrados o no estén duplicados.")
                    ->with('status', $success . " datos se importaron correctamente")
                    ->with('lines', $lines);
            } else {
                return back()
                    ->with('error', "Ningún dato se ha importado correctamente. Asegúrate que los datos del reporte o tabla, no estén registrados o no estén duplicados.")
                    ->with('lines', $lines);
            }
        }
    }
